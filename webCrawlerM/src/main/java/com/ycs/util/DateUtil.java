...
public class DateUtil {
    	...
    	public static final String lunarToSolarString(String lunar){
        	String solar="";
        	int year=Integer.parseInt(lunar.substring(0, 4));
        	int month=Integer.parseInt(lunar.substring(4,6));
        	int monthDay=Integer.parseInt(lunar.substring(6, 8));
        	int[] solarInt=new int[3];
        	try {
        		solarInt=DateUtil.lunarToSolar(year, month, monthDay, false);
			} catch (Exception e) {
				System.out.println("lunarToSolar Date Convert Exception");
				return lunar;
			}
        	
        	String[] solarStr = new String[3];
        	for(int i=0;i<3;i++){
        		solarStr[i]=solarInt[i]+"";
        		if(solarStr[i].length()==1){
        			solarStr[i]="0"+solarStr[i];
        		}
        		solar=solar+solarStr[i];
        	}
        	return solar;
        }
    	
        public static final String solarToLunarString(String solar){
        	String lunar="";
        	int year=Integer.parseInt(solar.substring(0, 4));
        	int month=Integer.parseInt(solar.substring(4,6));
        	int monthDay=Integer.parseInt(solar.substring(6, 8));
        	int[] lunarInt=new int [4];
        	try {
        		lunarInt=DateUtil.solarToLunar(year, month, monthDay);
			} catch (Exception e) {
				System.out.println("solarToLunar Date Convert Exception");
				return solar;
			}
        	
        	String[] lunarStr = new String[3];
        	for(int i=0;i<3;i++){
        		lunarStr[i]=lunarInt[i]+"";
        		if(lunarStr[i].length()==1){
        			lunarStr[i]="0"+lunarStr[i];
        		}
        		lunar=lunar+lunarStr[i];
        	}
        	return lunar;
        }
