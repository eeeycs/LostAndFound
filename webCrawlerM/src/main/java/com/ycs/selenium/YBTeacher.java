...
@Component
public class YBTeacher extends BasiceTestService{
	@Resource
...
//					}
	...
	public void startYb(int min){
		...
		for(int id=min;id<max;id++){
			Teacherinfo teacherinfo=(Teacherinfo) teacherinfoService.queryById(Teacherinfo.class, id);
			
			System.out.println(id);
			if(teacherinfo!=null){
				try {
					userName=teacherinfo.getTeacherId();
					userPass=teacherinfo.getIdcard();
					if(userPass.length()==18){
						userPass=userPass.substring(10,16);
					}else{
						userPass="123456";
					...
					System.out.println(teacherinfo.getTeacherId()+","+userPass);
					System.out.println("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");

					
					System.out.println(userName+"\t"+userPass);
					yb(teacherinfo.getId(),userName,userPass);
					//浏览器重启机制
					if(MyUtil.getFormatDate("mm").equals("00")||MyUtil.getFormatDate("mm").equals("30")){
						if(!MyUtil.getFormatDate("mm").equals(restartPre)){
							restart();
